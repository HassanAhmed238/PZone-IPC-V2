/**
 * Upload May 2026 invoices to Supabase (authenticated)
 * Run from the project root: node scripts/upload_may2026.mjs
 */
...
const invoicesPath = 'C:/Users/0255/.gemini/antigravity-ide/brain/b5d41a3a-60ef-4ac1-9a1b-ff2c78d03e81/scratch/may2026_invoices.json';
const allInvoices = JSON.parse(readFileSync(invoicesPath, 'utf-8'));
console.log(`Loaded ${allInvoices.length} invoices from JSON`);

// Step 1: Discover which columns the DB actually accepts
async function discoverValidColumns(sampleRow) {
  ...
  while (true) {
    ...
    if (!error) {
      ...
      console.log(`Schema discovery: ${fieldsToTry.length} columns accepted`);
    ...
    if (match) {
      rejected.push(match[1]);
      fieldsToTry = fieldsToTry.filter(f => f !== match[1]);
      continue;
    }
    
    // Check for status constraint violation
    if (error.message.includes('check') || error.message.includes('violates')) {
      console.warn('Constraint error (will fix data):', error.message);
      // Try without status to isolate
      if (fieldsToTry.includes('status')) {
        rejected.push('status');
        fieldsToTry = fieldsToTry.filter(f => f !== 'status');
        continue;
      }
    }
    
    console.error('Unexpected error during schema discovery:', error.message);
    process.exit(1);
  }
}
...
function cleanInvoice(inv) {
  ...
  for (const [k, v] of Object.entries(inv)) {
    if (validColumns.has(k)) cleaned[k] = v;
  }
  return cleaned;
}

const invoices = allInvoices.map(cleanInvoice);
...
if (fetchError) {
  console.error('Error fetching existing invoices:', fetchError.message);
  process.exit(1);
}

console.log(`Found ${existing?.length || 0} existing invoice rows\n`);
...
for (const inv of invoices) {
  ...
  if (matches && matches.length > 0) {
    toUpdate.push({ ...inv, id: matches[0].id });
  } else {
    toInsert.push(inv);
  }
}
...
for (const inv of toUpdate) {
  ...
  if (error) {
    console.error(`  ❌ Update ${inv.project_code} #${inv.invoice_number}: ${error.message}`);
    updateFail++;
  } else {
    console.log(`  ✅ Updated ${inv.project_code} #${inv.invoice_number || '-'}`);
    updateOk++;
  }
}
...
for (let i = 0; i < toInsert.length; i += 10) {
  ...
  if (error) {
    console.error(`  ❌ Batch ${Math.floor(i/10)+1}: ${error.message}`);
    // Try one-by-one
    for (const inv of batch) {
      const { data: d2, error: e2 } = await supabase.from('invoices').insert(inv).select();
      if (e2) {
        console.error(`    ❌ ${inv.project_code}: ${e2.message}`);
        insertFail++;
      } else {
        console.log(`    ✅ ${inv.project_code} #${inv.invoice_number || '-'}`);
        insertOk++;
      }
    }
  } else {
    ...
    for (const d of (data || batch)) {
      console.log(`  ✅ ${d.project_code} #${d.invoice_number || '-'}`);
    }
  }
}
...
console.log(`Total: ${updateOk + insertOk} successful out of ${allInvoices.length}`);
