...
async function run() {
  console.log("Reading Inventory Module migration SQL...");
  const migrationPath = path.join(__dirname, 'supabase/migrations/20260525110000_inventory_module.sql');
  const sql = fs.readFileSync(migrationPath, 'utf8');

  console.log("📡 Executing full migration via Supabase SQL API /pg endpoint...");
  ...
  if (res.ok) {
    console.log("✅ Inventory Migration executed successfully!");
  } else {
    const text = await res.text();
    console.log("Response status:", res.status);
    console.log("Response:", text);
    console.log("\n⚠️ Migration execution failed via /pg endpoint.");
  }
}
