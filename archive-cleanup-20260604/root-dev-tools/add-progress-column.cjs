...
async function run() {
  const { data, error } = await supabaseAdmin.rpc('exec_sql', { sql: 'ALTER TABLE public.ongoing_projects ADD COLUMN IF NOT EXISTS progress_sheet boolean DEFAULT false;' });
  console.log("Migration:", error || "Success");
}
